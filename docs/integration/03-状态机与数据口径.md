...
- `terminationPending` 是独立布尔标记，不属于来访状态枚举。长期拜访在场时申请终止，该标记为 `true`，`visitStatus` 继续保持来访中；完成当前出访后，标记清除且来访状态转为已终止。
...
stateDiagram-v2
  ...
  state 来访状态 {
    ...
    待来访 --> 已过期: 超过授权日期或所选时段
    待来访 --> 已终止: 提前终止且无人在场
    来访中 --> 已终止: terminationPending=true 后完成出访
  }
```
...
| 单次自动结束 | 不变 | 来访中 → 来访结束 | 到达预约结束时间；`endReason=TIMEOUT_AUTO_END` |
| 长期扫码离开 | 不变 | 来访中 → 待来访 | 关闭本次批次，授权期内可再次来访 |
| 自动过期 | 不变 | 待来访 → 已过期 | 超过授权日期或全部可用时段 |
| 提前终止（无人在场） | 不变 | 待来访 → 已终止 | 剩余进入授权作废 |
| 提前终止（仍在场） | 不变 | 来访中保持至出访，出访后 → 已终止 | `terminationPending=true`；立即禁止再次进入，保留当前出访权限；出访后清除标记 |
...
| `terminationPending` | 布尔值；仅表示长期授权已请求终止但仍有未闭合批次，不得作为 `visitStatus` 的第六个枚举 |
